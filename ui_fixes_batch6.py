# ── Batch 6: Augur sidebar, sessoes, multi-anexo, base de conhecimento ─────────
import re as _re_b6
...
def _b6_fix_augur_widget() -> None:
    ...
    if not _dash or "augurCard" not in _dash:
        ...
        return
    if "_b6AugurWidget" in _dash:
        print("[fixes_batch6] Augur widget ja atualizado")
        return

    _WIDGET = r"""
{# ── AUGUR WIDGET v4 ── #}
{% if current_client %}
<div class="card mb-3" id="augurCard" style="border:1px solid var(--mc-border);"><!-- _b6AugurWidget -->
  <div class="card-body p-0">
    ...
    <div style="display:flex;min-height:460px;">
      ...
      <div style="flex:1;display:flex;flex-direction:column;min-width:0;">
        <div id="augurChatArea" style="flex:1;overflow-y:auto;padding:1rem 1.25rem;display:flex;flex-direction:column;gap:.75rem;background:#fafafa;min-height:360px;">
...
(function(){
  ...
  function _scrollBot(){ const a=document.getElementById('augurChatArea'); a.scrollTop=a.scrollHeight; }
  ...
  window.selecionarAnexo = async function(input) {
    for(const file of Array.from(input.files)){
      ...
      if(['csv','xlsx','xls','txt'].includes(ext)){const t=await file.text();_augurAnexos.push({type:'csv',data:t.slice(0,50000),name:file.name});}
      else if(ext==='pdf'){const b=await new Promise(r=>{const rd=new FileReader();rd.onload=e=>r(e.target.result.split(',')[1]);rd.readAsDataURL(file)});_augurAnexos.push({type:'pdf',data:b,name:file.name});}
      else if(['png','jpg','jpeg','gif','webp'].includes(ext)){const m='image/'+(ext==='jpg'?'jpeg':ext);const b=await new Promise(r=>{const rd=new FileReader();rd.onload=e=>r(e.target.result.split(',')[1]);rd.readAsDataURL(file)});_augurAnexos.push({type:m,data:b,name:file.name});}
...
"""
    ...
    if "_b6AugurWidget" in new_dash:
        TEMPLATES["dashboard.html"] = new_dash
        if hasattr(templates_env.loader, "mapping"):
            templates_env.loader.mapping = TEMPLATES
        print("[fixes_batch6] ✅ Augur widget atualizado (sidebar, multi-anexo, sessoes)")
    else:
...
print("[fixes_batch6] ✅ Batch 6 aplicado")
