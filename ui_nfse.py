...
async def nfse_probe_codigo(request: _Req_nf, cod: str = "170200"):
    """Testa um TSCodTribNac específico contra o SNNFSE. Use ?cod=XXXXXX"""
    ...
    try:
        ...
        sub(inf, "serie", "1")
        sub(inf, "nDPS", "999")
        sub(inf, "dCompet", "2026-06-01")
        sub(inf, "tpEmit", "1")
        sub(inf, "cLocEmi", _NF_IBGE)
        ...
        try:
            ...
            except: body_json = r.text[:500]
            return _JR(content={"cod": cod, "amb": _NF_AMB, "status": r.status_code, "body": body_json})
        finally:
