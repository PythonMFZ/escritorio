...
# ============================
# MODULO: CREDITOS + CONSULTAS
# 1 crédito = R$1,00
# margem mínima: +50% em cima do custo Direct
# ============================
...
def ensure_consultas_tables() -> None:
    try:
        SQLModel.metadata.create_all(
            engine,
            tables=[
...
@app.on_event("startup")
def _startup_consultas_tables() -> None:
    ensure_consultas_tables()
...
def product_price_cents(p: QueryProduct) -> int:
    markup = max(50, int(p.markup_pct or 50))
    return int(math.ceil(p.provider_cost_cents * (1.0 + markup / 100.0)))


def get_or_create_wallet(session: Session, *, company_id: int, client_id: int) -> CreditWallet:
    w = session.exec(
...
def wallet_credit(session: Session, *, company_id: int, client_id: int, amount_cents: int, stripe_session_id: str) -> None:
    w = get_or_create_wallet(session, company_id=company_id, client_id=client_id)
...
def wallet_debit_or_402(session: Session, *, company_id: int, client_id: int, amount_cents: int, run_id: int, note: str) -> None:
    w = get_or_create_wallet(session, company_id=company_id, client_id=client_id)
...
def seed_credit_products(session: Session, company_id: int) -> None:
    if session.exec(select(QueryProduct).where(QueryProduct.company_id == company_id)).first():
        return
    # MVP (custos Direct em centavos; ajuste depois com base no seu cardápio)
    defaults = [
...
def directdata_call_stub(product_code: str, doc_digits: str) -> dict:
    # MVP stub (configuraremos Direct Data depois com seu token/base_url e mapeamento por produto)
    return {
        "provider": "directdata",
        "product_code": product_code,
        "doc": _mask_doc(doc_digits),
        "message": "MVP: integração Direct Data será configurada na próxima etapa (token/base_url + endpoints).",
    }
...
async def creditos_page(request: Request, session: Session = Depends(get_session)) -> HTMLResponse:
    ...
    if not client_id:
        set_flash(request, "Selecione um cliente para ver créditos.")
        return RedirectResponse("/", status_code=303)
    ...
    w = get_or_create_wallet(session, company_id=ctx.company.id, client_id=client.id)
    ledger = session.exec(
        select(CreditLedger)
...
async def creditos_checkout(request: Request, session: Session = Depends(get_session), pack: str = Form("50")) -> Response:
    ...
    packs = {"50": 5000, "100": 10000, "250": 25000, "500": 50000}  # cents
...
async def stripe_webhook(request: Request, session: Session = Depends(get_session)) -> Response:
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if company_id and client_id and credits:
            ...
            if not already:
                wallet_credit(session, company_id=company_id, client_id=client_id, amount_cents=credits * 100, stripe_session_id=session_id)
...
async def consultas_home(request: Request, session: Session = Depends(get_session)) -> HTMLResponse:
    ...
    seed_credit_products(session, ctx.company.id)
    ...
    enriched = [{"code": p.code, "label": p.label, "price_cents": product_price_cents(p)} for p in products]
    w = get_or_create_wallet(session, company_id=ctx.company.id, client_id=client.id)
...
async def consultas_product(request: Request, session: Session = Depends(get_session), code: str = "") -> HTMLResponse:
    ...
    w = get_or_create_wallet(session, company_id=ctx.company.id, client_id=client.id)
    pv = {"code": p.code, "label": p.label, "price_cents": product_price_cents(p)}
    return render("consulta_run.html", request=request, context={"title": p.label, "product": pv, "wallet_balance": f"{w.balance_cents/100:.2f}", "run": None})
...
async def consultas_run(request: Request, session: Session = Depends(get_session), code: str = "", doc: str = Form("")) -> HTMLResponse:
    ...
    p = session.exec(select(QueryProduct).where(QueryProduct.company_id == ctx.company.id, QueryProduct.code == code, QueryProduct.enabled == True)).first()  # noqa
    if not p:
        raise HTTPException(status_code=404, detail="Consulta não encontrada.")
    ...
    wallet_debit_or_402(
        ...
        amount_cents=price,
        run_id=run.id,
        note=f"Consulta: {p.label} ({p.code}) doc={_mask_doc(norm_doc)}",
    )

    try:
        payload = directdata_call_stub(p.code, norm_doc)
...
async def admin_consultas(request: Request, session: Session = Depends(get_session)) -> HTMLResponse:
    ...
    enriched = [{
        ...
        "enabled": p.enabled,
        "price_cents": product_price_cents(p),
    } for p in products]
...
async def admin_consultas_save(
    ...
    if p:
        ...
        session.add(QueryProduct(company_id=ctx.company.id, code=code, label=label, provider_cost_cents=cost_cents, markup_pct=markup, enabled=True, created_at=utcnow(), updated_at=utcnow()))
    session.commit()
