# ============================================================================
# PATCH — Restaura e reposiciona Augur no dashboard
# ============================================================================
# O patch anterior removeu o Augur sem reinserir. Este patch restaura
# o widget completo em largura total no dashboard.
# DEPLOY: adicione ao final do app.py
# ============================================================================

_AUGUR_WIDGET_FULL = r"""
{% if current_client %}
<div class="row g-3 mb-3">
  <div class="col-12">

    {# ── AUGUR ── #}
    <div class="card" id="augurCard" style="border:1px solid var(--mc-border);">
      <div class="card-body p-0">
        ...
        <div class="d-flex align-items-center gap-2 p-3" style="border-bottom:1px solid var(--mc-border);">
          ...
          <div style="flex:1;">
            <div class="fw-bold" style="font-size:.92rem;">Augur
              <span id="augurSessaoTitulo" style="font-weight:400;font-size:.78rem;color:var(--mc-muted);margin-left:.5rem;"></span>
            </div>
        ...
        <div style="display:flex;height:500px;">
          ...
          <div id="augurSidebar" style="width:240px;border-right:1px solid var(--mc-border);overflow-y:auto;padding:.5rem;background:#f8f9fa;flex-shrink:0;">
            ...
            <div id="augurSessaoLista">
              <div class="muted small" style="padding:.5rem;font-size:.72rem;">Carregando...</div>
            </div>
          </div>
          ...
          <div style="flex:1;display:flex;flex-direction:column;min-width:0;">
            <div id="augurChatArea" style="flex:1;overflow-y:auto;padding:1rem 1.25rem;display:flex;flex-direction:column;gap:.75rem;background:#fafafa;">
              <div style="text-align:center;color:var(--mc-muted);font-size:.82rem;padding:2rem 0;" data-placeholder>
                Carregando sessões...
              </div>
            </div>
    ...
    <div class="card mt-3" id="baseConhecimentoCard" style="border:1px solid var(--mc-border);">
      <div class="card-body p-0">
        <div class="d-flex align-items-center gap-2 p-3" style="border-bottom:1px solid var(--mc-border);">
...
<style>
  ...
  .aug-sessao-item.ativa div{opacity:1!important;}
</style>
...
(function(){
  ...
  async function _carregarSessoes() {
    try {
      ...
      d.sessoes.forEach(s => {
        ...
        el.className = 'aug-sessao-item' + (_sessaoId === s.id ? ' ativa' : '');
        el.dataset.id = s.id;
        el.innerHTML = `<div style="font-weight:600;font-size:.75rem;">${_esc(s.titulo)}</div><div style="font-size:.65rem;opacity:.6;">${s.updated_at||''}</div>`;
        el.onclick = () => _abrirSessao(s.id, s.titulo);
        el.ondblclick = () => _renomear(s.id, s.titulo, el);
        lista.appendChild(el);
      });
      if (!_sessaoId && d.sessoes.length) {
        _abrirSessao(d.sessoes[0].id, d.sessoes[0].titulo);
      }
    } catch(e) { _novaSessao(); }
  }
  ...
  async function _abrirSessao(id, titulo) {
    ...
    try {
      ...
      if (!d.mensagens||!d.mensagens.length) {
        area.innerHTML = '<div style="text-align:center;color:var(--mc-muted);font-size:.82rem;padding:2rem;">Nenhuma mensagem ainda.</div>';
      ...
      _scrollBottom();
    } catch(e) { area.innerHTML = '<div style="text-align:center;color:var(--mc-muted);padding:2rem;">Erro ao carregar.</div>'; }
  }

  function _novaSessao() {
    _sessaoId = null;
  ...
  window.augurNovaConversa = _novaSessao;

  async function _renomear(id, titulo, el) {
    const novo = prompt('Renomear conversa:', titulo);
    ...
    if (d.ok) {
      ...
      if (_sessaoId===id) document.getElementById('augurSessaoTitulo').textContent=d.titulo;
    }
  }

  function _adicionarSessaoNaLista(id, titulo) {
    const lista = document.getElementById('augurSessaoLista');
    // Remove "Nenhuma conversa"
    ...
    el.dataset.id = id;
    el.innerHTML = `<div style="font-weight:600;font-size:.75rem;">${_esc(titulo)}</div><div style="font-size:.65rem;opacity:.6;">${new Date().toISOString().slice(0,10)}</div>`;
    el.onclick = () => _abrirSessao(id, titulo);
    el.ondblclick = () => _renomear(id, titulo, el);
    lista.insertBefore(el, lista.firstChild);
    // Remove ativa de outros
  ...
  window.augurSend = async function() {
    ...
    _renderMsg('user', q+(_augurAnexo?` [📎 ${_augurAnexo.name}]`:''), null, hora, true);
    _showTyping();
    const payload = {question:q||'(Analise o arquivo)', session_id:_sessaoId||0};
    ...
    try {
      ...
      if (d.session_id) {
        if (!_sessaoId) {
          _sessaoId = d.session_id;
          document.getElementById('augurSessaoTitulo').textContent = d.session_titulo||'';
          _adicionarSessaoNaLista(d.session_id, d.session_titulo||'Nova conversa');
        }
      }
  ...
  window.selecionarAnexo=async function(input){
    ...
    if(['csv','xlsx','xls','txt'].includes(ext)){const txt=await file.text();_augurAnexo={type:'csv',data:txt.slice(0,50000),name:file.name};}
    else{const reader=new FileReader();const tipo=ext==='pdf'?'pdf':['png','jpg','jpeg','gif','webp'].includes(ext)?'image/'+(ext==='jpg'?'jpeg':ext):null;if(!tipo){alert('Tipo não suportado.');return;}reader.onload=e=>{_augurAnexo={type:tipo,data:e.target.result.split(',')[1],name:file.name};};reader.readAsDataURL(file);}
  ...
  window.removerAnexo=function(){_augurAnexo=null;document.getElementById('augurAnexoPreview').style.display='none';document.getElementById('augurFileInput').value='';};
  ...
  async function _baseCarregar(){
    ...
    if(!d.docs||!d.docs.length){lista.innerHTML='<div class="muted small">Nenhum documento ainda. Adicione arquivos para o Augur usar.</div>';return;}
    ...
    }catch(e){document.getElementById('baseDocLista').innerHTML='<div class="muted small">Erro ao carregar.</div>';}}
  ...
  window.baseSalvar=async function(){
    ...
    try{
      ...
      else{fb.innerHTML=`<div class="alert alert-danger py-1 mb-0">${d.erro||'Erro ao salvar.'}</div>`;}
  ...
  window.baseDeletar=async function(id,btn){
    ...
    try{await fetch('/api/base-conhecimento/'+id,{method:'DELETE'});_baseCarregar();}catch(e){}
  };
...
# Injeta no dashboard logo após o bloco do painel (card de boas vindas)
_dash = TEMPLATES.get("dashboard.html", "")
if _dash and "augurCard" not in _dash:
    # Insere antes do primeiro {% if smart_alerts %} ou antes do Painel de Saúde
    if "Painel de Saúde Financeira" in _dash:
        _dash = _dash.replace(
            "Painel de Saúde Financeira",
            _AUGUR_WIDGET_FULL + "\nPainel de Saúde Financeira",
            1
        )
    elif "smart_alerts" in _dash:
        _dash = _dash.replace(
            "{% if smart_alerts %}",
            _AUGUR_WIDGET_FULL + "\n{% if smart_alerts %}",
            1
        )
    elif "{% endblock %}" in _dash:
        _dash = _dash.replace(
            "{% endblock %}",
            _AUGUR_WIDGET_FULL + "\n{% endblock %}",
            1
    ...
    print("[fix_augur_restaura] ✅ Augur restaurado no dashboard")
elif _dash and "augurCard" in _dash:
    print("[fix_augur_restaura] ✅ Augur já está no dashboard")
else:
    print("[fix_augur_restaura] ⚠️ dashboard.html não encontrado")

if hasattr(templates_env.loader, "mapping"):
    templates_env.loader.mapping = TEMPLATES

print("[fix_augur_restaura] ✅ Patch completo")
